...
Write-Host "$YELLOW  Follow Public Account [JianBingGuoZiJuan AI] $NC"
Write-Host "$YELLOW  Join us for more Cursor tips and AI knowledge (script is free, follow the account to join the group for more tips)  $NC"
Write-Host "$YELLOW  [IMPORTANT] This tool is free. If it helps you, please follow [JianBingGuoZiJuan AI]  $NC"
Write-Host "$BLUE================================$NC"
Write-Host ""
...
Write-Host "$YELLOW[IMPORTANT NOTE]$NC Latest 0.47.x (supported)"
Write-Host ""
    ...
    if ($process) {
        ...
        while ($retryCount -lt $MAX_RETRIES) {
            ...
            if ($retryCount -ge $MAX_RETRIES) {
                Write-Host "$RED[ERROR]$NC Cannot close $processName after $MAX_RETRIES attempts"
                Get-ProcessDetails $processName
...
# Use new functions when generating IDs
...
function Update-MachineGuid {
    try {
        ...
        if (($backupFile -ne $null) -and (Test-Path $backupFile)) {
            Write-Host "$YELLOW[RECOVER]$NC Restoring from backup..."
            $restoreResult = Start-Process "reg.exe" -ArgumentList "import", "`"$backupFile`"" -NoNewWindow -Wait -PassThru
            
            if ($restoreResult.ExitCode -eq 0) {
                Write-Host "$GREEN[RECOVERY SUCCESS]$NC Original registry value restored"
            } else {
                Write-Host "$RED[ERROR]$NC Recovery failed, please manually import backup file: $backupFile"
            }
        } else {
...
try {
    ...
    # Directly update MachineGuid without asking
    Update-MachineGuid
    # Display results
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "$GREEN================================$NC"
    Write-Host "$YELLOW  Follow public account [JianBingGuoZiJuan AI] for more Cursor tips and AI knowledge (script is free, follow for group access)  $NC"
    Write-Host "$GREEN================================$NC"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "$YELLOW[QUERY]$NC Do you want to disable Cursor auto-update feature?"
